...
make "$coefficiency" install check_physical_closure 2>&1 | tee "$log_dir"/install | sed -e "$build_clutter" -e "$install_clutter"

printf 'Production system built--ready to start GUI test in another session.\n' > /dev/tty

printf '\n# cgi and cli tests\n\n'
make "$coefficiency" --output-sync=recurse cgi_tests cli_tests 2>&1 | tee "$log_dir"/cgi-cli | sed -e "$build_clutter" -e "$cli_cgi_clutter"

printf '\n# system test\n\n'
make "$coefficiency" system_test 2>&1 | tee "$log_dir"/system-test | sed -e "$build_clutter" -e "$install_clutter"
...
make "$coefficiency" all build_type=so_test USE_SO_ATTRIBUTES=1 2>&1 | tee "$log_dir"/so_test | sed -e "$build_clutter"

printf '\n# cgi and cli tests in libstdc++ debug mode\n\n'
make "$coefficiency" --output-sync=recurse cgi_tests cli_tests build_type=safestdlib 2>&1 | tee "$log_dir"/cgi-cli-safestdlib | sed -e "$build_clutter" -e "$cli_cgi_clutter"
...
printf '\n# xrc tests\n\n'
java -jar /opt/lmi/third_party/rng/jing.jar -c xrc.rnc *.xrc 2>&1 | tee "$log_dir"/xrc
...
printf '\n# schema tests\n\n'
/opt/lmi/src/lmi/test_schemata.sh 2>&1 | tee "$log_dir"/schemata | sed -e "$schemata_clutter"
