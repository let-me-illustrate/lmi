...
void AccountValue::PerformSpecAmtStrategy()
{
    ...
    for(int j = 0; j < BasicValues::Length; ++j)
        ...
        DeathBfts_->set_specamt(currency(std::max(m, z)), j, 1 + j); // round(currency)?
//      DeathBfts_->set_specamt(currency(round_specamt()(std::max(m, z))), j, 1 + j); // round(currency)?
        if
            (  j == InforceYear
...
void AccountValue::PerformSupplAmtStrategy()
{
    for(int j = 0; j < BasicValues::Length; ++j)
        ...
        currency z = CalculateSpecAmtFromStrategy(j, 0, explicit_value, strategy);
        // Shouldn't rounding be done upstream? Was it?
//      DeathBfts_->set_supplamt(currency(round_specamt()(std::max(m, z))), j, 1 + j); // round(currency)?
        DeathBfts_->set_supplamt(currency(std::max(m, z)), j, 1 + j); // round(currency)?
        }
}
