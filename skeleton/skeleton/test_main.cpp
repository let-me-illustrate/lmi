...
// $Id: test_main.cpp,v 1.1.1.1 2004-05-15 19:59:18 chicares Exp $
...
int cpp_main(int argc, char* argv[])
{
    ...
    catch(boost_modified::test::test_tools_exception const&)
        {
        std::cout << "\n**** previous test error is fatal" << std::endl;
        // Reset so we don't get two messages.
        boost_modified::test::test_tools_errors = 0;
        result = boost_modified::exit_test_failure;
        }

    if(boost_modified::test::test_tools_errors)
        {
        std::cout
            << "\n**** "
            << boost_modified::test::test_tools_errors
...
// Revision History
//  2004-05-05 GWC removed carriage returns.
