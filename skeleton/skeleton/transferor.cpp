...
// $Id: transferor.cpp,v 1.1.1.1 2004-05-15 19:59:31 chicares Exp $
...
// TODO ?? Could boost::any be used instead?
...
bool Transferor::PerformTransfer(transfer_direction td)
{
    ...
    if     (0 != (button       = dynamic_cast<wxButton      *>(control)))
        ...
        {
        wxMessageBox("Unrecognized control.");
        return false;
        }
}
...
bool TransferBool(transfer_direction td, std::string& data, T& control)
{
    if(td == from_string_to_control)
        {
        control.SetValue(value_cast<bool>(data));
        }
    else
        {
        data = value_cast<std::string>(control.GetValue());
        }
    return true;
}
...
bool TransferInt(transfer_direction td, std::string& data, T& control)
{
    if(td == from_string_to_control)
        {
        control.SetValue(value_cast<int>(data));
        }
    else
        {
        data = value_cast<std::string>(control.GetValue());
...
bool Transfer(transfer_direction td, std::string& data, wxScrollBar& control)
{
    if(td == from_string_to_control)
        {
        control.SetThumbPosition(value_cast<int>(data));
        }
    else
        {
        data = value_cast<std::string>(control.GetThumbPosition());
        }
    return true;
}
