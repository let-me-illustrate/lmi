...
<!--
    ...
    $Id: offshore_private_placement.xsl,v 1.27 2007-06-12 08:28:52 etarassov Exp $
-->
<!DOCTYPE stylesheet [
<!ENTITY nbsp "&#xA0;">
]>
      ...
      <xsl:with-param name="top-block">
        <xsl:if test="$include-underwriter">
          <fo:block padding-bottom="1em">
            ...
            <xsl:value-of select="$illustration/scalar/InsCoName"/>
            and is located at
            <fo:block><xsl:value-of select="$illustration/scalar/MainUnderwriterAddress"/></fo:block>
          </fo:block>
        </xsl:if>
        <fo:block>
