...
// $Id: inputillus.cpp,v 1.38 2008-08-09 19:22:16 chicares Exp $
...
IllusInputParms::IllusInputParms()
    ...
    ,GenAcctIntRate                   ("") // Intentionally blank.
    ,SepAcctIntRate                   (".08")
    ,NewLoan                          ("0")
    ,Withdrawal                       ("0")
    ,FlatExtra                        ("0")
...
void IllusInputParms::propagate_fund_allocations_from_string()
{
    ...
    if(v.size() < NumberOfFunds)
        {
        v.insert(v.end(), NumberOfFunds - v.size(), r_fund(0));
        }
    FundAllocs = v;
...
// for which no change is proposed.
    ...
    ascribe("InforceNetExperienceReserve"  , &IllusInputParms::InforceNetExperienceReserve);
    ascribe("InforceYtdNetCoiCharge"       , &IllusInputParms::InforceYtdNetCoiCharge     );
