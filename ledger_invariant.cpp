...
//  SmokerDistinct          = 0;
    ...
    if(b->ProductData_)
        ...
        PolicyMktgName                 = p["PolicyMktgName"                 ].str();
        PolicyLegalName                = p["PolicyLegalName"                ].str();
        PolicyForm                     = p["PolicyForm"                     ].str();
        InsCoShortName                 = p["InsCoShortName"                 ].str();
        InsCoName                      = p["InsCoName"                      ].str();
        InsCoAddr                      = p["InsCoAddr"                      ].str();
        InsCoStreet                    = p["InsCoStreet"                    ].str();
        InsCoPhone                     = p["InsCoPhone"                     ].str();
        MainUnderwriter                = p["MainUnderwriter"                ].str();
        MainUnderwriterAddress         = p["MainUnderwriterAddress"         ].str();
        CoUnderwriter                  = p["CoUnderwriter"                  ].str();
        CoUnderwriterAddress           = p["CoUnderwriterAddress"           ].str();

        AvName                         = p["AvName"                         ].str();
        CsvName                        = p["CsvName"                        ].str();
        CsvHeaderName                  = p["CsvHeaderName"                  ].str();
        NoLapseProvisionName           = p["NoLapseProvisionName"           ].str();
        InterestDisclaimer             = p["InterestDisclaimer"             ].str();
        GuarMortalityFootnote          = p["GuarMortalityFootnote"          ].str();

        AccountValueFootnote           = p["AccountValueFootnote"           ].str();
        AttainedAgeFootnote            = p["AttainedAgeFootnote"            ].str();
        CashSurrValueFootnote          = p["CashSurrValueFootnote"          ].str();
        DeathBenefitFootnote           = p["DeathBenefitFootnote"           ].str();
        InitialPremiumFootnote         = p["InitialPremiumFootnote"         ].str();
        NetPremiumFootnote             = p["NetPremiumFootnote"             ].str();
        OutlayFootnote                 = p["OutlayFootnote"                 ].str();
        PolicyYearFootnote             = p["PolicyYearFootnote"             ].str();

        ADDFootnote                    = p["ADDFootnote"                    ].str();
        ChildFootnote                  = p["ChildFootnote"                  ].str();
        SpouseFootnote                 = p["SpouseFootnote"                 ].str();
        TermFootnote                   = p["TermFootnote"                   ].str();
        WaiverFootnote                 = p["WaiverFootnote"                 ].str();

        MinimumPremiumFootnote         = p["MinimumPremiumFootnote"         ].str();
        PremAllocationFootnote         = p["PremAllocationFootnote"         ].str();

        ProductDescription             = p["ProductDescription"             ].str();
        StableValueFootnote            = p["StableValueFootnote"            ].str();
        NoVanishPremiumFootnote        = p["NoVanishPremiumFootnote"        ].str();
        RejectPremiumFootnote          = p["RejectPremiumFootnote"          ].str();
        ExpRatingFootnote              = p["ExpRatingFootnote"              ].str();
        MortalityBlendFootnote         = p["MortalityBlendFootnote"         ].str();
        HypotheticalRatesFootnote      = p["HypotheticalRatesFootnote"      ].str();
        SalesLoadRefundFootnote        = p["SalesLoadRefundFootnote"        ].str();
        NoLapseFootnote                = p["NoLapseFootnote"                ].str();
        MarketValueAdjFootnote         = p["MarketValueAdjFootnote"         ].str();
        ExchangeChargeFootnote0        = p["ExchangeChargeFootnote0"        ].str();
        CurrentValuesFootnote          = p["CurrentValuesFootnote"          ].str();
        DBOption1Footnote              = p["DBOption1Footnote"              ].str();
        DBOption2Footnote              = p["DBOption2Footnote"              ].str();
        ExpRatRiskChargeFootnote       = p["ExpRatRiskChargeFootnote"       ].str();
        ExchangeChargeFootnote1        = p["ExchangeChargeFootnote1"        ].str();
        FlexiblePremiumFootnote        = p["FlexiblePremiumFootnote"        ].str();
        GuaranteedValuesFootnote       = p["GuaranteedValuesFootnote"       ].str();
        CreditingRateFootnote          = p["CreditingRateFootnote"          ].str();
        MecFootnote                    = p["MecFootnote"                    ].str();
        MidpointValuesFootnote         = p["MidpointValuesFootnote"         ].str();
        SinglePremiumFootnote          = p["SinglePremiumFootnote"          ].str();
        MonthlyChargesFootnote         = p["MonthlyChargesFootnote"         ].str();
        UltCreditingRateFootnote       = p["UltCreditingRateFootnote"       ].str();
        MaxNaarFootnote                = p["MaxNaarFootnote"                ].str();
        PremTaxSurrChgFootnote         = p["PremTaxSurrChgFootnote"         ].str();
        PolicyFeeFootnote              = p["PolicyFeeFootnote"              ].str();
        AssetChargeFootnote            = p["AssetChargeFootnote"            ].str();
        InvestmentIncomeFootnote       = p["InvestmentIncomeFootnote"       ].str();
        IrrDbFootnote                  = p["IrrDbFootnote"                  ].str();
        IrrCsvFootnote                 = p["IrrCsvFootnote"                 ].str();
        MortalityChargesFootnote       = p["MortalityChargesFootnote"       ].str();
        LoanAndWithdrawalFootnote      = p["LoanAndWithdrawalFootnote"      ].str();
        PresaleTrackingNumber          = p["PresaleTrackingNumber"          ].str();
        CompositeTrackingNumber        = p["CompositeTrackingNumber"        ].str();
        InforceTrackingNumber          = p["InforceTrackingNumber"          ].str();
        InforceCompositeTrackingNumber = p["InforceCompositeTrackingNumber" ].str();
        InforceNonGuaranteedFootnote0  = p["InforceNonGuaranteedFootnote0"  ].str();
        InforceNonGuaranteedFootnote1  = p["InforceNonGuaranteedFootnote1"  ].str();
        InforceNonGuaranteedFootnote2  = p["InforceNonGuaranteedFootnote2"  ].str();
        InforceNonGuaranteedFootnote3  = p["InforceNonGuaranteedFootnote3"  ].str();
        NonGuaranteedFootnote          = p["NonGuaranteedFootnote"          ].str();
        MonthlyChargesPaymentFootnote  = p["MonthlyChargesPaymentFootnote"  ].str();
        }
